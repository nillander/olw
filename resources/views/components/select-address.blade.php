...
<select id="address_id" name="address_id" {{ $disabled ? 'disabled' : '' }} {!! $attributes->merge(['class' =>
    ...
    <option {{ isset($selectedValue) && $option->id == $selectedValue->id ? 'selected' : '' }} value="{{ $option->id }}"
        >{{ $option->city.' - '.$option->state }}</option>
    @endforeach
</select>
